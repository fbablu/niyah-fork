...
- **Default-deny** for unmatched collections; server-managed collections (`transactions`, `groupSessions`, `revenue`, `walletAudits`, …) take **admin-SDK-only writes** — some still allow scoped reads (`transactions` owner-read, `groupSessions` participant-read), so don't assume "no client read."
