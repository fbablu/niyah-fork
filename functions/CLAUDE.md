...
- **Type-check / lint:** `cd functions && npm run lint` (`tsc --noEmit`). Build: `npm run build`. Tests live beside source (`src/*.test.ts`) and run under the **Node built-in runner** (`node:test` + `tsx`) via `pnpm test:functions` from the repo root — **NOT jest** (jest's `testMatch` skips `functions/`). **Don't** run `firebase deploy` — Fardeen does that.
