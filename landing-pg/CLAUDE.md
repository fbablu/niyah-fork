...
- **Hosted legal lives here:** `app/legal/page.tsx` (index) + `app/legal/{privacy,terms}/page.tsx` + shared `app/legal/layout.tsx` → served at `niyah.live/legal`, `/legal/privacy`, `/legal/terms` (clean URLs, no `.html`). Keep these in sync with in-app `src/components/LegalContentView.tsx`; bump `CURRENT_LEGAL_VERSION` in the app if terms change materially.
